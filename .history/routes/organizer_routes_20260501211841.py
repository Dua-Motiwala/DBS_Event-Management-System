...
def dashboard():
    ...
    # Calculate total registrations for all events owned by this organizer
    total_registrations = db.session.query(db.func.count(Registration.regid))\
        .join(Event)\
        .filter(Event.userid == current_user.userid)\
        .scalar() or 0
        
    return render_template('dashboards/organizer.html', 
                           events=my_events, 
                           today=today, 
...
def delete_event(event_id):
    ...
    return redirect(url_for('organizer.dashboard'))
@organizer_bp.route('/finish-event/<int:event_id>', methods=['POST'])
@login_required
@organizer_required
def finish_event(event_id):
    ...
    from datetime import datetime
    if event.eventdate > datetime.utcnow().date():
        flash('Event has not occurred yet. You can only finish it after the event date.', 'warning')
        return redirect(url_for('organizer.dashboard'))
    
    try:
        event.status = 'Finished'
        db.session.commit()
        flash('Event marked as finished!', 'success')
    except Exception as e:
        db.session.rollback()
